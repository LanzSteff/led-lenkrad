 ...
 * web-site: http://scream3r.org | http://code.google.com/parameter/java-simple-serial-connector/
 */
package ledStripe.jssc;
...
public class SerialPort {
	...
	public boolean setParams(int baudRate, int dataBits, int stopBits,
		...
		return serialInterface.setParams(portHandle, baudRate, dataBits,
				stopBits, parity, true, true);
	}
	...
	public boolean setParams(int baudRate, int dataBits, int stopBits,
		...
		return serialInterface.setParams(portHandle, baudRate, dataBits,
				stopBits, parity, setRTS, setDTR);
	}

	/**
	 * Purge of input and output buffer. Required flags shall be sent to the
	 * input. Variables with prefix <b>"PURGE_"</b>, for example
	 * <b>"PURGE_RXCLEAR"</b>. Sent parameter "flags" is additive value, so
	 * addition of flags is allowed. For example, if input or output buffer
	 * shall be purged, parameter <b>"PURGE_RXCLEAR | PURGE_TXCLEAR"</b>. <br>
	...
	/**
	 * Set events mask. Required flags shall be sent to the input. Variables
	 * with prefix <b>"MASK_"</b>, shall be used as flags, for example
	 * <b>"MASK_RXCHAR"</b>. Sent parameter "mask" is additive value, so
	 * addition of flags is allowed. For example if messages about data receipt
	 * and CTS and DSR status changing shall be received, it is required to set
	 * the mask - <b>"MASK_RXCHAR | MASK_CTS | MASK_DSR"</b>
	 * 
	 * @return If the operation is successfully completed, the method returns
	 *         true, otherwise false
	 * 
	...
	public boolean setEventsMask(int mask) throws SerialPortException {
		...
		if (SerialNativeInterface.getOsType() == SerialNativeInterface.OS_LINUX
				...
				|| SerialNativeInterface.getOsType() == SerialNativeInterface.OS_MAC_OS_X) {// since
																							// 0.9.0
			linuxMask = mask;
			if (mask > 0) {
				maskAssigned = true;
		...
		if (!returnValue) {
			throw new SerialPortException(portName, "setEventsMask()",
					SerialPortException.TYPE_CANT_SET_MASK);
		}
		if (mask > 0) {
			maskAssigned = true;
		} else {
	 ...
	 * 
	 * @return Method returns events mask as int type variable. This variable is
	 *         an additive value
	 * 
	 * @throws SerialPortException
	 */
	...
	/**
	 * Read Hex string from port (example: FF OA FF). Separator by default is a
	 * space
	 * 
	 * @param byteCount
	 *            count of bytes for reading
	 * 
	 * @return byte array with "byteCount" length converted to Hexadecimal
	 *         String
	 * 
	 * @throws SerialPortException
	 * 
	...
	/**
	 * Read Hex string from port with setted separator (example if separator is
	 * "::": FF::OA::FF)
	 * 
	 * @param byteCount
	 *            count of bytes for reading
	 ...
	 *            count of bytes for reading
	 * 
	 * @return String array with "byteCount" length and Hexadecimal String
	 *         values
	 * 
	 * @throws SerialPortException
	 * 
	 ...
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port
	 * 
	 * @throws SerialPortException
	 * 
	 * @since 0.8
	 ...
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port converted to String
	 * 
	 * @throws SerialPortException
	 * 
	 * @since 0.8
	 ...
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port converted to Hex String
	 * 
	 * @throws SerialPortException
	 * 
	 * @since 0.8
	...
	/**
	 * Read all available bytes from port like a Hex String with setted
	 * separator
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port converted to Hex String
	 ...
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port converted to Hex String array
	 * 
	 * @throws SerialPortException
	 * 
	 * @since 0.8
	...
	/**
	 * Read all available bytes from port like a int array (values in range from
	 * 0 to 255)
	 * 
	 * @return If input buffer is empty <b>null</b> will be returned, else byte
	 *         array with all data from port converted to int array
	 * 
	 * @throws SerialPortException
	 * 
	 * @since 0.8
	...
	private void checkPortOpened(String methodName) throws SerialPortException {
		if (!portOpened) {
			throw new SerialPortException(portName, methodName,
	...
	/**
	 * Add event listener. MsgObject of <b>"SerialPortEventListener"</b> type shall
	 * be sent to the method. This object shall be properly described, as it
	 * will be in charge for handling of occurred events. This method will
	 * independently set the mask in <b>"MASK_RXCHAR"</b> state if it was not
	 * set beforehand
	 * 
	 * @throws SerialPortException
	 */
	public void addEventListener(SerialPortEventListener listener)
		...
		if (!eventListenerAdded) {
			...
			throw new SerialPortException(portName, "addEventListener()",
					SerialPortException.TYPE_LISTENER_ALREADY_ADDED);
		}
	}
	 ...
	 * Add event listener. MsgObject of <b>"SerialPortEventListener"</b> type shall
	 * be sent to the method. This object shall be properly described, as it
	 * will be in charge for handling of occurred events. Also events mask shall
	 * be sent to this method, to do it use variables with prefix <b>"MASK_"</b>
	 * for example <b>"MASK_RXCHAR"</b>
	 * 
	...
	public boolean removeEventListener() throws SerialPortException {
		...
		if (!eventListenerAdded) {
			throw new SerialPortException(portName, "removeEventListener()",
					SerialPortException.TYPE_CANT_REMOVE_LISTENER);
		}
			...
			if (eventThread.isAlive()) {
				try {
					...
					throw new SerialPortException(
							portName,
							"removeEventListener()",
							SerialPortException.TYPE_LISTENER_THREAD_INTERRUPTED);
				}
			}
		}
			...
			while (!threadTerminated) {
				...
				for (int i = 0; i < eventArray.length; i++) {
					if (eventArray[i][0] > 0 && !threadTerminated) {
						eventListener.serialEvent(new SerialPortEvent(portName,
								eventArray[i][0], eventArray[i][1]));
					}
				}
			}
		}
				...
				for (int i = 0; i < eventArray.length; i++) {
					...
					if (eventType > 0 && !super.threadTerminated) {
						...
						case MASK_RLSD: /* DCD */
							...
							break;
						case MASK_RXCHAR:
							if (((mask & MASK_RXCHAR) == MASK_RXCHAR)
									&& (eventValue > 0)) {
								sendEvent = true;
							}
							break;
						/*
						 * case MASK_RXFLAG: //Do nothing at this moment
						 * if(((mask & MASK_RXFLAG) == MASK_RXFLAG) &&
						 * (eventValue > 0)){ sendEvent = true; } break;
						 */
						case MASK_TXEMPTY:
							if (((mask & MASK_TXEMPTY) == MASK_TXEMPTY)
									&& (eventValue == 0) && interruptTxChanged) {
								sendEvent = true;
							}
							break;
						}
						if (sendEvent) {
							eventListener.serialEvent(new SerialPortEvent(
									portName, eventType, eventValue));
						}
					}
				}
